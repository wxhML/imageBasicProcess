...
def removeFile(path):
    fielPath = path
    fileList = os.listdir(fielPath)
    for fileJson in fileList:
        allList = glob.glob(fielPath + "/" + fileJson + "/*.json")
        for single in allList:
            # os.rmdir(single) #删除目录
            os.remove(single)  # 删除单个文件


if __name__=="__main__":
    # removeFile("F:/军牌/多样车牌/双层军牌") #删除json文件，递归是一层
    ...
    for pathSingle in fileList:
        pathSingle = "/nfsroot/" + pathSingle
        print(pathSingle)
